        ...
        execute: function() {
            ...
            SqlQuery = (function () {
                ...
                SqlQuery._columns = function (key, value, fromQuery) {
                    ...
                    var keyAlias = key.trim().split(' ').pop(), valueAlias = value.trim().split(' ').pop(), havingIndex = fromQuery.toLowerCase().indexOf('having'), having = "", aliasRegex = /%(\w+)/g, concatExpr = "";
                    if (key.match(/^'.*'$/) && key.match(aliasRegex)) {
                        concatExpr = 'concat(' + key.replace(aliasRegex, "', $1, '").replace(/, ''/g, '') + ')';
                        key = keyAlias = lodash_1.default.uniq(key.match(aliasRegex).map(function (k) { return k.slice(1); })).join(', ');
                    }
                    if (havingIndex !== -1) {
